...
void shadow_map_demo() {
	...
	shared_ptr<StandardModel> model_ptr = StandardModel::getModel("models/room_thickwalls.obj", vector<pair<string, string>> {
	...
	shared_ptr<VertexTextureToolModel> cube_ptr = VertexTextureToolModel::getTool(vec4(0, 0, 256, 256), GL_TRIANGLES, scene_ptr->getTexture(0));
	cube_ptr->addSpirit(BaseModelSpirit::getModelSpirit(Spirit::getImmortalSpirit(vec3(), quat(), vec3())));
	vector<GLfloat> data = {
		-1.0f, -1.0f, 0.0f,
		1.0f, -1.0f, 0.0f,
		-1.0f,  1.0f, 0.0f,
	...
	cube_ptr->addSpirit(BaseModelSpirit::getModelSpirit(Spirit::getImmortalSpirit()));
