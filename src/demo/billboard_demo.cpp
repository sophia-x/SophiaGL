...
void billboard_demo() {
	...
	// Create DebugModel
	shared_ptr<VertexColorToolModel> cube_ptr = VertexColorToolModel::initTool(vec4(0, 0, WIDTH, HEIGHT), GL_TRIANGLES);
	...
	manager.addTool("CUBE", cube_ptr->getInstance(Spirit::getImmortalSpirit(vec3(), quat(), vec3(0.2f))));

	shared_ptr<BillboardToolModel> billboard_ptr = BillboardToolModel::initTool(vec4(0, 0, WIDTH, HEIGHT), vec4(0.04, 0.3, 1, 0.7),
	...
	manager.addTool("Billboard", billboard_ptr->getInstance(base_ptr));
	...
	while (manager.next()) {
		...
		manager.step(delta, vector<string> {}, vector<string> {"CUBE", "Billboard"});
	}
}